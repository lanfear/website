...
angular.module('myApp.directives')
  ...
  .directive('slideSource', ['$compile', '$timeout', '$swipe', '$interval', '$rootScope', function ($compile, $timeout, $swipe, $interval, $rootScope) {
    return {
        transclude: true,
        restrict: 'A',
        compile: function (element, attrs, transclude) {
            return function ($scope, $element, $attr) {
                ...
                $scope.initializeViewer = function () {
                  ...
                  $scope.menuClass = (typeof attrs.menuClass == "undefined" ? "" : attrs.menuClass);
                  ...
                  $scope.infoClass = (typeof attrs.infoClass == "undefined" ? "" : attrs.infoClass);
                ...
                $scope.createViewer = function () {
                  ...
                  var htmlText = '<div class="' + $scope.menuClass + '">' +
                                 '<div class="' + $scope.previousClass + '" ng-click="_navPrev();" style="cursor: pointer;"></div>' +
                                 '<div class="' + $scope.nextClass + '" ng-click="_navNext();" style="cursor: pointer;"></div>';
 
                  if ($scope.infoClass !== '') {
                    htmlText += '<div id="' + $scope.getId("info") + '" class="' + $scope.infoClass + '" ng-click="_getInfo();" style="cursor: pointer;" title="info"></div>';
                  }
                ...
                $scope.getBackgroundUrl = function () {
                  if($scope.backgroundSource === null) return null;
                  var dataArray = $scope.getDataArray();
                ...
                $scope._getInfo = function () {
                    // callback
                    if (typeof $scope[$attr.getInfo] === 'function') {
                        var result = $scope[$attr.getInfo]($scope[$scope._index]);
                    }
                }
    ...
    .directive('jmdAudioPlayer', function($rootScope) {
        return {
            restrict: 'E',
            scope: {},
            ...
            template:   '    <div class="media-player-wrapper">' +
                        '        <div class="form-group">' +
                        '            <label>volume: <input min="0" max="1" step="0.01" type="range" ng-value="audio.volume" /></label>' +
                        '        </div>' +
                        '        <div class="form-group">' +
                        ...
                        '            <label>time: <input min="0" max="{{audio.duration}}" step="0.01" type="range" ng-value="audio.currentTime" /></label>' +
                        '        </div>' +
                        '        <div class="form-group now-playing">' +
                        '            <p>Now Playing: <img width="30" height="30" src="{{info.image}}" /></p>' +
                        '            <p>{{info.tracknum}} - {{info.trackname}}</p>' +
                        '        </div>' +
                        '    </div>'
